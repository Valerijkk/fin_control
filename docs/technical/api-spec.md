...
Собственного backend в текущем проекте нет. Все данные — локально (SQLite, SharedPreferences). При необходимости добавить минимальный демо-сервер для практик сниффа — описать отдельно в [docs/practices/](../practices/).
