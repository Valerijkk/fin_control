# 🗄️ Модели данных FinControl (SQLite)

> 📌 Модели соответствуют **одному приложению FinControl**. Реализация таблиц и репозиториев — в `lib/data/db.dart`, `lib/domain/models/`, `lib/domain/repositories/`. Обзор архитектуры: [architecture.md](architecture.md).
...
| `is_income` | INTEGER | 0 — расход, 1 — доход |
| `date` | TEXT | ISO8601 |
| `photo_path` | TEXT | Путь к фото чека (nullable) |

### 🏷️ Категории

| Тип | Хранение |
|-----|----------|
| Встроенные | `lib/core/categories.dart` |
| Пользовательские | `SharedPreferences` через `CategoryStore` |
...
| `avg_rate` | REAL | Средняя цена входа в базовой валюте |
| `updated_at` | TEXT | ISO8601 |
...
|---------|--------|----------|
| `exchange_operations` | по `created_at` | Сортировка истории |
| `portfolio_transactions` | по `created_at` | Сортировка истории |
